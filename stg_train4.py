...
from stg import SingleTemporalGraphClassifier
from torch import nn

# 设置设备，优先使用 GPU（如果可用）
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

model = SingleTemporalGraphClassifier(in_channels=3, hidden_channels=32, embedding_dim=64, num_classes=4, num_layers=2).to(device)
...
torch.save(model.state_dict(), f'stg_model4.pth')
